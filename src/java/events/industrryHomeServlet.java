 ...
 * @author HP
 */
public class industrryHomeServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet industrryHomeServlet</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet industrryHomeServlet at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
                    ...
                    else  if(event.contains("calender")) 
                    {
                        HttpSession session = request.getSession(false);
                        session.setAttribute("NameUniversity", ""+univercity);
                        response.sendRedirect("http://localhost:8080/plymouthapp/calender/colUniCalender.jsp");
                    }
                    else 
                    {
                   ...
                   /* else if (univercity.equals("University-of-Peradeniya") && event.contains("professional")) {    
                    ...
                    else  if(event.contains("calender") && univercity.equals("University-of-Peradeniya")) {
                        response.sendRedirect("http://localhost:8080/plymouthapp/calender/paradeniyacalender.jsp");
                    }
                    else  if(event.contains("calender") && univercity.equals("NSBM")) {
                        response.sendRedirect("http://localhost:8080/plymouthapp/calender/nsbmUniCalander.jsp");
                    }*/
                    
                    
                 
                   /* if(univercity.contains("University-of-Colombo")&& userType.contains("industry"))
                        {
                        response.sendRedirect("http://localhost:8080/plymouthapp/events/colPost.jsp");
                        }
                    else if(univercity.contains("University-of-Peradeniya")&& userType.contains("industry"))
                        {
                        response.sendRedirect("http://localhost:8080/plymouthapp/events/paradeniyaPost.jsp");
                        
                        }
                      else if (univercity.contains("NSBM")&& userType.contains("industry"))
                        {
                         response.sendRedirect("http://localhost:8080/plymouthapp/events/nsbmEvent.jsp");
